...
def run_fusion(live_features: pd.DataFrame) -> pd.DataFrame:
    ...
    if live_features.empty:
        ...
        return pd.DataFrame(columns=['game_id', 'home_team', 'away_team', 'model_probability', 'market_probability', 'edge', 'risk_level'])
    ...
    final_game_prediction = pd.DataFrame(prediction_rows, columns=[
        'game_id', 'home_team', 'away_team', 'model_probability', 'market_probability', 'edge', 'risk_level'
    ])
